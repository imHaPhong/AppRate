...
function updateApartment(txtPropertyType,txtBedroom,txtCurrentDateTime,txtMonthlyPrice,cbxFurnitureType,txtNote,txtReporterName){
    var db = openDatabase('RentalApartmentDatabase', '1.0', 'Database for RentalZ app', 2 * 1024 * 1024);
    db.transaction(function (tx) {
        var sql = "UPDATE apartment SET property_type = ?, bedroom = ?, current_date_time = ?, monthly_price = ?, furniture_type = ?, note = ?, reporter_name = ? WHERE id = ?";
        tx.executeSql(sql, [txtPropertyType, txtBedroom, txtCurrentDateTime, txtMonthlyPrice, cbxFurnitureType, txtNote, txtReporterName, update_id]);
    });
}
