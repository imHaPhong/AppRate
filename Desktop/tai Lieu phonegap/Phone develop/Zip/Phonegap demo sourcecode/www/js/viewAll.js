...
$("#btnPopulate").click(function(event) {
    ...
    db.transaction(function(tx) {
        ...
        tx.executeSql('CREATE TABLE apartment (id integer primary key autoincrement, property_type, bedroom, current_date_time, monthly_price, furniture_type1, furniture_type2, furniture_type3, note, reporter_name)');
        var sql = "INSERT INTO apartment(property_type, bedroom, current_date_time, monthly_price, furniture_type1, furniture_type2, furniture_type3, note, reporter_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        tx.executeSql(sql, ["Bungalow", "Studio", "2019-11-04T03:06", "19000000", "Furnished", "", "Twilight Sparkle"]);
        tx.executeSql(sql, ["House", "Two", "2019-10-26T03:06", "6000000", "Furnished", "Furnished", "Furnished", "", "Twilight Sparkle"]);
        tx.executeSql(sql, ["Flat", "One", "2019-11-04T03:06", "3000000", "Half Furnished", "Half Furnished", "Half Furnished", "", "Twilight Sparkle"]);
        tx.executeSql(sql, ["Flat", "Three", "2019-11-04T03:06", "2000000", "Unfurnished", "Half Furnished", "Half Furnished", "", "Twilight Sparkle"]);
        tx.executeSql(sql, ["House", "One", "2019-11-06T03:06", "4000000", "Half Furnished", "Half Furnished", "Half Furnished", "", "Twilight Sparkle"]);
    });
    refreshTable();
...
function loadDataToTable(results) {
    ...
    for (i = 0; i < len; i++) {
        ...
        var furnitureType1 = row.insertCell(5);
        var furnitureType2 = row.insertCell(6);
        var furnitureType3 = row.insertCell(7);
        var note = row.insertCell(8);
        var reporterName = row.insertCell(9);
        var edit = row.insertCell(10);
        var editN = row.insertCell(11);
        var remove = row.insertCell(12);
        ...
        furnitureType1.innerHTML = results.rows.item(i).furniture_type1;
        furnitureType2.innerHTML = results.rows.item(i).furniture_type2;
        furnitureType3.innerHTML = results.rows.item(i).furniture_type3;
